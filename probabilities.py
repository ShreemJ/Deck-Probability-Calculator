...
def combo_set_probability(set_config, card_counts, card_types, combos_constraints, extra_info=False):
    ...
    def generate_counts_with_replacements(cards, hand_size, current_counts, index, expression, replacements, deck_counts, result):
        if index == len(cards):
            if sum(current_counts.values()) == hand_size:
                hand = []
                for card, count in current_counts.items():
...
def single_trial_combo_type(deck_counts, default_hand_size, card_types, combo_name, combo_data):
...
def monte_carlo_combo_type_probability(deck_counts, default_hand_size, card_types, combos, trials=10000, max_workers=None):
    ...
    def run_trials_for_combo(combo_name, combo_data):
        hits = 0
        for _ in range(trials):
            hits += single_trial_combo_type(deck_counts, default_hand_size, card_types, combo_name, combo_data)
        return hits

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(run_trials_for_combo, name, data): name
